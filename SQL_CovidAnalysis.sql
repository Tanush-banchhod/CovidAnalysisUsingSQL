...
select Location, MAX(total_cases) AS HighestCases, population, MAX(total_cases/population)*100 as PercentagePolupationInfected
from PortfolioProject..CovidDeaths
...
select SUM(new_cases) as Total_Cases_on_that_day,SUM(cast(new_deaths as int)) as Total_deaths_on__that_day
, SUM(cast(new_deaths as int))/SUM(new_cases) *100 as DeathPercentage
from PortfolioProject..CovidDeaths
where continent is not null
order by 1,2

select location,SUM(cast(new_deaths as int)) as Total_deaths_on__that_day
from PortfolioProject..CovidDeaths
